...
metadata = {
    'protocolName': 'Version 1 S9 Station B ThermoFisher (630µl sample input)',
    'author': 'Marco & Giada',
    'apiLevel': '2.3'
}
...
def run(ctx):
    ctx.comment("Station B ThermoFisher protocol for {} samples".format(NUM_SAMPLES))
    ...
    # PCR plate
    pcr_plate = ctx.load_labware('opentrons_96_aluminumblock_nest_wellplate_100ul', '2', 'chilled elution plate on block for Station C')
...
resuming.')
    ...
    def elute(vol):
        ...
        delay(4, 'Incubating on temperature module at 80°C', ctx)
        ...
        tempdeck.deactivate()
        ctx.pause("Move deepwell plate back from temperature module on 3 to magnetic module on 4")
    ...
    # --- RUN -----------------------------------------------------------------
    delay(5, 'Waiting before magnetic module activation', ctx)
    magdeck.engage(height=magheight)
    ...
    magdeck.disengage()
    ctx.comment("Move chilled elution plate on block (slot 2) to Station C")
    # -------------------------------------------------------------------------
